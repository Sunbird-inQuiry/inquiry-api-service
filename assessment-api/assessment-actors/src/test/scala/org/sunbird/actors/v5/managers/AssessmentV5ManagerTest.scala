...
class AssessmentV5ManagerTest extends BaseSpec with PrivateMethodTester with MockFactory with Matchers {
  ...
  //  assert(thrown.getErrorCode == ErrorCodes.ERR_BAD_REQUEST.name())
  }
  ...
  "hideEditorStateAns" should "remove 'answer' from 'options' in JSON with 'answer'" in {
    ...
    //(mockNode.getMetadata _).expects().returning(Map("editorState" -> editorStateWithAnswer).asJava)
    ...
    // Check that 'answer' is removed from each option
...
//    )
//
//    val assessments: util.List[util.Map[String, AnyRef]] = assessmentsData.map {
//      case (assessmentTs, batchId, collectionId, userId, attemptId, contentId) =>
//        createAssessment(assessmentTs, batchId, collectionId, userId, attemptId, contentId).asJava
//    }.asJava
//
//    // Mock necessary dependencies
//    (oec.graphService _).expects().returning(mock[GraphService])
//
//    // Call the calculateScore method
//    AssessmentV5Manager invokePrivate calculateScore(privateList, assessments)
//
//    // Add your assertions here based on the expected behavior of the calculateScore function
//  }
...
//  def createAssessments(data: util.Map[String, AnyRef]): util.List[util.Map[String, AnyRef]] = {
...
//  "populateMultiCardinality" should "set score and pass in edata based on user response" in {
//
//    val responseMap = new util.HashMap[String, AnyRef]()
//    responseMap.put(AssessmentConstants.CORRECT_RESPONSE, Map(AssessmentConstants.VALUE -> List(1, 2, 3).asJava).asJava)
//    responseMap.put(AssessmentConstants.MAPPING, List(
//      Map(AssessmentConstants.RESPONSE -> 1, AssessmentConstants.OUTCOMES -> Map(AssessmentConstants.SCORE -> 5.0d).asJava).asJava,
//      Map(AssessmentConstants.RESPONSE -> 2, AssessmentConstants.OUTCOMES -> Map(AssessmentConstants.SCORE -> 3.0d).asJava).asJava,
//      Map(AssessmentConstants.RESPONSE -> 3, AssessmentConstants.OUTCOMES -> Map(AssessmentConstants.SCORE -> 2.0d).asJava).asJava
//    ).asJava)
//
//    val edataMap = new util.HashMap[String, AnyRef]()
//    edataMap.put(AssessmentConstants.RESVALUES, new util.ArrayList[util.ArrayList[util.Map[String, AnyRef]]]())
//    val resValuesList = edataMap.get(AssessmentConstants.RESVALUES).asInstanceOf[util.ArrayList[util.ArrayList[util.Map[String, AnyRef]]]]
//    resValuesList.add(new util.ArrayList[util.Map[String, AnyRef]]())
//
//    AssessmentV5Manager populateMultiCardinality(responseMap,edataMap,10)
//
//    edataMap.get(AssessmentConstants.SCORE) shouldEqual 5.0d.asInstanceOf[AnyRef]
//    edataMap.get(AssessmentConstants.PASS) shouldEqual AssessmentConstants.YES
//  }
